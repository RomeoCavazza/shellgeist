...
Document unique regroupant les audits techniques : bugs de flux, appels d’outils (parsing / fallbacks), qualité de code (anti-slop), et verdicts des derniers runs. Dernière mise à jour : état du code actuel.

**Docs associées**  
- **`results.md`** — Transcript du dernier run + verdicts / audit en bas de fichier.  
- **`specification.txt`** — Spécification technique (architecture, fichiers).  
- **`audit-results-deep.md`** — Analyse détaillée de runs passés (syntaxe, run_shell, correctifs).

**Références code** : `backend/shellgeist/agent/loop.py`, `backend/shellgeist/agent/orchestrator.py`, `backend/shellgeist/agent/parsing/parser.py`, `backend/shellgeist/runtime/policy.py`, `backend/shellgeist/runtime/paths.py`, `backend/shellgeist/llm/prompt.py`, `backend/shellgeist/runtime/server.py`, `nvim/lua/shellgeist/init.lua`.
...
| **Statut** | ✅ **Fait** — (1) Règle 8 dans le prompt (chemins ambigus) : en cas de chemin ambigu, utiliser le chemin relatif complet depuis la racine (ex. `backend/shellgeist/__init__.py`). (2) Dans `policy.py`, `is_ambiguous_path_result()` + dans `record_outcome` : ne pas compter cette erreur comme échec, pas d’ajout à `blocked_call_hashes` → le modèle peut réessayer avec un chemin complet. |
...
| **edit_file guard_blocked** | Quand edit_file retourne `syntax_error_after_edit`, la boucle n’injectait pas de feedback → le modèle réessayait le même edit → BLOCKED_REPEAT. **Correctif** : injecter un message utilisateur « Utilise write_file pour remplacer tout le fichier » après une telle observation (loop.py). | — |
| **Patch par touche** | Run script.py : run_shell échoue (NameError shutil) → modèle a fait write_file réécriture complète. Correctif : _is_simple_fix_failure ; repair_feedback privilégie edit_file avec diff minimal pour NameError/ImportError ; edit_file autorisé sur la cible en mode strict. | — |
...
| `backend/shellgeist/llm/prompt.py` | Règles 1–8 (dont exécute-le, vérification .py après write_file, chemins ambigus). |
...
## Partie 5 — Dernier run (results.md)

Transcript et verdicts détaillés : **`docs/results.md`** (section « Audit / Verdicts » en bas).

**Résumé (run cube 3D — mars 2025)** : Deux tours. Tour 1 : write_file (contenu tronqué à `polygons = {`) + read_file, **aucun run_shell** ; réponse « Voici le contenu de terminal_cube.py… » (pas de slope_trailing) ; vérif manuelle → SyntaxError `'(' was never closed`. Tour 2 (user colle l’erreur) : write_file réécriture complète + read_file, toujours **aucun run_shell** ; vérif manuelle → SyntaxError `Function parameters cannot be parenthesized` (l.99) — signature `def draw_triangle(..., (x1, y1), ...)` = **Python 2**, invalide en Python 3. **Causes** : règle 7 (py_compile + python3) non appliquée ou run avant déploiement ; slope_trailing non utilisé (réponse générique) ; modèle génère du code Python 2. **Pistes** : confirmer déploiement règle 7 + fix slope_trailing ; rappel prompt/repair « Python 3 uniquement — pas de tuple unpacking dans les paramètres ».

Résumés de runs antérieurs : Ping (création + repair) OK. Cube simple : write OK mais py_compile/python3 non exécutés. Cube 3D (autre run) : write OK, run_shell ×5 puis BLOCKED_REPEAT — correctifs is_failed_result, hints repair, évitement double message erreur UI.

**Run edit_file (results.md)** : run_shell échoue (TypeError) → le modèle tente edit_file → `guard_blocked — syntax_error_after_edit` (edit.py refuse d’écrire un contenu avec erreur de syntaxe) → même appel réessayé → BLOCKED_REPEAT_TOOL. **Duplication** : la boucle injecte un feedback « utilise write_file » quand run_shell échoue, mais pas quand edit_file échoue avec syntax_error_after_edit. **Correctif** : après une observation edit_file contenant `syntax_error_after_edit`, injecter un message utilisateur « Utilise write_file pour remplacer tout le fichier » (loop.py). Voir `docs/results.md` section « Audit — edit_file et duplication de logique ».

**Run épique (script.py, patch par touche)** : write_file → read_file → run_shell KO (NameError shutil). **edit_file OK** : patch minimal (+ `import shutil`). Puis edit_file KO (syntax_error_after_edit) → write_file réécriture complète avec échappements indésirables (`\"\"\` au lieu de `"""`) → SyntaxError l.14 → message « Tentatives de correction : 2 ». Positif : patch par touche fonctionne. À traiter : normalisation write_file (dés-échappement).

**Nouveau run (énormes progrès)** : write_file → read_file → run_shell KO (shutil). **edit_file OK** (patch riche : import shutil + corrections indentation / print). Puis pas de 2e run_shell ; modèle enchaîne edit_file (rejet x2), read_file x5 → BLOCKED_REPEAT_TOOL. Pistes : après edit_file réussi, inciter run_shell pour revalider ; rappel si read_file répété ; assouplir seuil read_file en repair. Voir results.md « Audit — Nouveau run ».

---

## Partie 6 — Slope et prompt : générique, scalable

Ne pas lutter contre la nature du modèle (répondre en bloc code). Accueillir le ```code``` et le convertir en write_file de façon générique. Éviter : inférence par mot-clé (cube→cube.py, ping→ping.py). Faire : slope sans strict_target uniquement quand le goal contient exactement un chemin .py (extract_file_references) ; prompt allégé (une phrase sur escape en string, pas de spec 3D lourde). Détail dans loop.py (_single_py_file_ref_from_goal) et prompt.py (règles 4–5).
